...
void main() {
    vec2 end_pos = endPos;
    vec2 start_pos = startPos;

    if(gl_VertexID == 1 || gl_VertexID == 2) {
        gl_Position = getClipSpacePosition(end_pos);
        return;
    }
    // World Space offset
    float scaleFactor = 5000.0f;
    vec2 end2start = normalize(start_pos - end_pos);
    mat2 rotateM = rotate(u_arrowAngle, gl_VertexID == 0);
    vec2 rotatedVector = (rotateM * end2start);
    vec2 offsetedPos = end_pos + rotatedVector * u_arrowLength / scaleFactor;
    gl_Position = getClipSpacePosition(offsetedPos);
