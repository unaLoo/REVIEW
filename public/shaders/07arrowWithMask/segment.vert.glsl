...
void main() {
    ...
    if(startPosSS.x<-1.0f || startPosSS.x>1.0f || startPosSS.y<-1.0f || startPosSS.y>1.0f || endPosSS.x<-1.0f || endPosSS.x>1.0f || endPosSS.y<-1.0f || endPosSS.y>1.0f){
        return;
    }
    ...
    // if(parity == 0) {
    //     gl_Position = startPosCS;
    // } else {
    //     gl_Position = endPosCS;
    // }
    // opacity = abs(2.0f * float(parity) - 1.0f);
}