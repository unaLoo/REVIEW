...
bool validExtentCheck(vec2 pos, vec4 extent) {
    // float e = 1.0f;
    // vec4 nextent = extent + vec4(e, e, -e, -e);
    // if(pos.x <= nextent.x  || pos.x >= nextent.z || pos.y <= nextent.y || pos.y >= nextent.w) {
    //     return true;
    // }
...
void main() {
    ...
    //     out_endPos = newPos;
    // }
    if(all(lessThan(abs(uvSpeed), vec2(0.000001f))) || !validExtentCheck(newPos, cExtent)) {
        out_endPos = nowPos;
    } else {
        out_endPos = newPos;
    }
