...
@group(0) @binding(1) var<uniform> mapUniform: MapUniformBlock;
...
fn vMain(input: VertexInput) -> VertexOutput {
    ...
    let nodeStartX = floor((box[input.instanceIndex * 4 + 0] - tileUniform.tileBox[0]) / tileUniform.sectorRange.x);
    let nodeStartY = floor((box[input.instanceIndex * 4 + 1] - tileUniform.tileBox[1]) / tileUniform.sectorRange.y);
    let nodeEndX = ceil((box[input.instanceIndex * 4 + 2] - tileUniform.tileBox[0]) / tileUniform.sectorRange.x);
    let nodeEndY = ceil((box[input.instanceIndex * 4 + 3] - tileUniform.tileBox[1]) / tileUniform.sectorRange.y);
    ...
    let vertex = vertices[input.vertexIndex] / mapUniform.dimensions;

    var output: VertexOutput;
    output.position = vec4f(vertex * 2.0 - 1.0, 0.0, 1.0);
    output.level = f32(level[input.instanceIndex]);
    return output;
}

@fragment
fn fMain(input: VertexOutput) -> @location(0) u32 {

    return u32(input.level);
    // return vec4f(input.level / 255.0);
}