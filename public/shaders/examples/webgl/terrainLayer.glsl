...
void main() {
    ...
    if (intervalN + intervalE + intervalS + intervalW != 4 * intervalM) {

        // fragColor = vec4(0.39, 0.04, 0.04, 1.0);
        fragColor = vec4(contourColor / 255.0, 1.0 - depthRate);
    } else {
        ...
        fragColor = vec4(intervalColor, 1.0 - pow(2.71828, -20.0 * (1.0 - depthRate))) * 0.8;
        // fragColor = vec4(vec3(0.5), 1.0 - pow(2.71828, -0.5 * (1.0 - depthRate)));
        // fragColor = vec4(intervalColor, 1.0);
        // fragColor = vec4(vec3(1.0 - depthRate), 1.0);
        // fragColor = vec4(1.0 - depthRate);
