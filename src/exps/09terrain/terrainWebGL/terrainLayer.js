...
export default class TerrainLayer {
    ...
    async init(gl) {
        ...
        this.normalShader = await createShader(gl, '/scratchSomething/terrainWebGL/shader/normal.glsl')
        this.lodMapShader = await createShader(gl, '/scratchSomething/terrainWebGL/shader/lodMap.glsl')
        this.dLodMapShader = await createShader(gl, '/scratchSomething/terrainWebGL/shader/dLodMap.glsl')
        this.terrainLineShader = await createShader(gl, '/scratchSomething/terrainWebGL/shader/terrainMeshLine.glsl')
        this.terrainMeshShader = await createShader(gl, '/scratchSomething/terrainWebGL/shader/terrainMesh.glsl')
        this.terrainShader = this.asLine ? this.terrainLineShader : this.terrainMeshShader
        this.showShader = await createShader(gl, '/scratchSomething/terrainWebGL/shader/terrainLayer.glsl')

        const demImageBitmap = await loadImage('/scratchSomething/terrainWebGL/image/dem.png')
        const contourPaletteBitmap = await loadImage('/scratchSomething/terrainWebGL/image/contourPalette1D.png')
