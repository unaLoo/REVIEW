...
const modelLayer = {
    ...
    config: {
        lightPosition: [120.53794466757358, 32.08061107103058, 5000],
    ...
    async onAdd(map, gl) {
        ...
        this.init_showScreenSizeTexture()
    ...
    render(gl, matrix) {
        ...
        let lightPos = MercatorCoordinate.fromLngLat([this.config.lightPosition[0], this.config.lightPosition[1]], this.config.lightPosition[2])
        ...
        let lighterPosMatrix = mat4.lookAt(mat4.create(), lightPos, [mercatorPos.x, mercatorPos.y, 0], [0, 1, 0])
        let lighterViewMatrix = mat4.invert(mat4.create(), lighterPosMatrix)
        let lighterProjMatrix = mat4.perspective(mat4.create(), Math.PI / 2, gl.canvas.width / gl.canvas.height, 0.1, 10000)
        gl.useProgram(this.shadowProgram)
        gl.viewport(0, 0, 512, 512)
        this.meshes.forEach((mesh, index) => {
            // model matrix :: make model vertices and light position in mapbox mercator coordinate system
            let lighterModelMatrix = this.calcModelMatrix(mesh, mercatorPos, scale, mesh.needRotate)
            ...
            gl.uniformMatrix4fv(
                gl.getUniformLocation(this.shadowProgram, 'u_model'),
                false,
                lighterModelMatrix
            )
        ...
        this.render_showScreenSizeTexture(this.meshes[0].depthTexture)
...
*/
    ...
    afterLoad() {
        ...
        ///////// pass for shadow texture //////////
        const shadowVertShaderSource = `#version 300 es
        ...
        `;
        let shadowProgram = this.shadowProgram = createProgramFromSource(gl, shadowVertShaderSource, shadowFragShaderSource)


        ///////// pass for render model ///////////
        ...
        this.meshes = []
        this.meshes.push(this.init4ModelMesh(this.init4Shadow(supportMesh)))
        this.meshes.push(this.init4ModelMesh(this.init4Shadow(bladesMesh)))
        this.meshes[1].needRotate = true

        ////////// mesh object has depthTexture, depthFramebuffer, vao4shadow and vao, texture, needRotate
    ...
    init4Shadow(mesh) {
        ...
        const depthFramebuffer = mesh.depthFramebuffer = gl.createFramebuffer()
        gl.bindFramebuffer(gl.FRAMEBUFFER, depthFramebuffer)
        ...
        let vao = mesh.vao4shadow = gl.createVertexArray();
        gl.bindVertexArray(vao);
        gl.enableVertexAttribArray(0);
        gl.bindBuffer(gl.ARRAY_BUFFER, vertPosBuffer);
    ...
    initGUI() {
        ...
        gui.add(this.config.lightPosition, 0, 120, 121, 0.001).name('longitude').onChange(value => this.config.lightPosition[0] = value)
        gui.add(this.config.lightPosition, 1, 31.5, 32.5, 0.001).name('latitude').onChange(value => this.config.lightPosition[1] = value)
        gui.add(this.config.lightPosition, 2, 3000, 10000, 10).name('altitude').onChange(value => this.config.lightPosition[2] = value)
    ...
    init_showScreenSizeTexture(texture) {
        const vs = `#version 300 es
        vec4[] vertices = vec4[4](
            vec4(-1.0, -1.0, 0.0, 0.0),
            vec4(1.0, -1.0, 1.0, 0.0),
        ...
        `
        this.showScrenSizeTextureProgram = createProgramFromSource(this.gl, vs, fs)

    },
    render_showScreenSizeTexture(texture) {

        const gl = this.gl
        gl.useProgram(this.showScrenSizeTextureProgram)
        gl.activeTexture(gl.TEXTURE0)
        gl.bindTexture(gl.TEXTURE_2D, texture)
    ...
    drawLighter(matrix) {
        const gl = this.gl
        // const lightPos = [120.53794466757358, 32.08061107103058, 5000]
