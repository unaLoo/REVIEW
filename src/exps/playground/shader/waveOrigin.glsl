...
float getwaves(vec2 position, int iterations) {
  ...
  // 先不考虑多权波叠加，只考虑单种波形
    ...
    // // 随机方向
    // vec2 randDir = vec2(sin(position.x), cos(position.y));

    // // wave和导数
    // vec2 res = wavedx(position, randDir, frequency, iTime * timeMultiplier + wavePhaseShift);

    // // shift position around according to wave drag and derivative of the wave
    // position += randDir * res.y * weight * DRAG_MULT;
...
vec3 normal(vec2 pos, float e, float depth) {
  ...
  return normalize(cross(a - vec3(pos.x - e, getwaves(pos.xy - ex.xy, ITERATIONS_NORMAL) * depth, pos.y), a - vec3(pos.x, getwaves(pos.xy + ex.yx, ITERATIONS_NORMAL) * depth, pos.y + e)));
}
...
vec3 getRay(vec2 fragCoord) {
  ...
  return createRotationMatrixAxisAngle(vec3(0.0, -1.0, 0.0), 3.0 * ((NormalizedMouse.x + 0.5) * 2.0 - 1.0)) * createRotationMatrixAxisAngle(vec3(1.0, 0.0, 0.0), 0.5 + 1.5 * (((NormalizedMouse.y == 0.0 ? 0.27 : NormalizedMouse.y) * 1.0) * 2.0 - 1.0)) * proj;
}
...
void main() {
  ...
  // fragColor = vec4(waterHitPos, 1.0);
}
#endif